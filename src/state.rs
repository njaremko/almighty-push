...
impl StateManager {
    ...
    /// Mark a PR as merged (permanent)
    pub fn mark_pr_as_merged(&self, change_id: &str) -> Result<()> {
        let mut state = self.load()?;
        state.merged_pr_change_ids.insert(change_id.to_string());
        self.write_state(&state)?;
        Ok(())
    }

    /// Mark a PR as closed (permanent)
    pub fn mark_pr_as_closed(&self, change_id: &str) -> Result<()> {
        let mut state = self.load()?;
        state.closed_pr_change_ids.insert(change_id.to_string());
        self.write_state(&state)?;
        Ok(())
    }
